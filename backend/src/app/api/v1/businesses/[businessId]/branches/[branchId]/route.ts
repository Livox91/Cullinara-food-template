...
import { UpdateBranchSchema } from "@/server/modules/branches/branch.schemas";
...
export async function GET(request: Request, context: Context) {
  return apiHandler(request, async ({ requestId }) => {
    const { actor, branchId } = await actorFor(
      request,
      context,
      "business.read",
    );
    return apiOk(await branchService.get(actor, branchId), requestId);
  });
}
...
export async function PATCH(request: Request, context: Context) {
  return apiHandler(request, async ({ requestId }) => {
    ...
    return apiOk(
      await branchService.update(actor, branchId, input, requestId),
      requestId,
    );
  });
}
