...
import { UpsertSpecialHoursSchema } from "@/server/modules/branches/branch.schemas";
...
export async function PUT(request: Request, context: Context) {
  return apiHandler(request, async ({ requestId }) => {
    ...
    const input = UpsertSpecialHoursSchema.parse(await readJson(request));
    return apiOk(
      await branchService.upsertSpecialHours(actor, branchId, input, requestId),
      requestId,
    );
  });
}
